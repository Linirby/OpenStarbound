...
OpenGlRenderer::~OpenGlRenderer() {
  for (auto& effect : m_effects) {
    ...
    for (auto& ubo : effect.second.uniformBuffers) {
      if (ubo.second.bufferId)
        glDeleteBuffers(1, &ubo.second.bufferId);
    }
  }
...
void OpenGlRenderer::loadEffectConfig(String const& name, Json const& effectConfig, StringMap<String> const& shaders) {
  if (auto effect = m_effects.ptr(name)) {
    ...
    for (auto& ubo : effect->uniformBuffers) {
      if (ubo.second.bufferId)
        glDeleteBuffers(1, &ubo.second.bufferId);
    }
  ...
  for (auto const& p : effectConfig.getObject("effectUniformBuffers", {})) {
    GLuint blockIndex = glGetUniformBlockIndex(m_program, p.first.utf8Ptr());
    if (blockIndex == GL_INVALID_INDEX) {
      Logger::warn("OpenGL20 effect parameter '{}' has no associated uniform block, skipping", p.first);
    } else {
      GLuint bindingPoint = p.second.getUInt("binding", 0);
      size_t bufferSize = p.second.getUInt("size", 0);

      glUniformBlockBinding(m_program, blockIndex, bindingPoint);

      EffectUniformBuffer uniformBuffer;
      uniformBuffer.bindingPoint = bindingPoint;
      uniformBuffer.bufferSize = bufferSize;

      glGenBuffers(1, &uniformBuffer.bufferId);
      if (uniformBuffer.bufferId == 0)
        throw RendererException::format("Could not generate uniform buffer for block '{}' in effect '{}'", p.first, name);

      glBindBuffer(GL_UNIFORM_BUFFER, uniformBuffer.bufferId);
      glBufferData(GL_UNIFORM_BUFFER, uniformBuffer.bufferSize, nullptr, GL_DYNAMIC_DRAW);
      glBindBuffer(GL_UNIFORM_BUFFER, 0);

      glBindBufferBase(GL_UNIFORM_BUFFER, uniformBuffer.bindingPoint, uniformBuffer.bufferId);

      effect.uniformBuffers[p.first] = uniformBuffer;
    }
  }
...
void OpenGlRenderer::setEffectUniformBufferData(String const& effectName, String const& blockName, void const* data, size_t size) {
  auto find = m_effects.find(effectName);
  if (find == m_effects.end())
    return;

  Effect& effect = find->second;
  auto ptr = effect.uniformBuffers.ptr(blockName);
  if (!ptr || ptr->bufferId == 0)
    return;
  ...
  size_t uploadSize = min(size, ptr->bufferSize);
  glBindBuffer(GL_UNIFORM_BUFFER, ptr->bufferId);
  glBufferSubData(GL_UNIFORM_BUFFER, 0, uploadSize, data);
  glBindBuffer(GL_UNIFORM_BUFFER, 0);
}

void OpenGlRenderer::setEffectUniformBuffer(String const& effectName, String const& blockName, void const* data, size_t size) {
  setEffectUniformBufferData(effectName, blockName, data, size);
}

void OpenGlRenderer::setEffectUniformBuffer(String const& effectName, String const& blockName, ByteArray const& data) {
  setEffectUniformBufferData(effectName, blockName, data.ptr(), data.size());
}
...
bool OpenGlRenderer::switchEffectConfig(String const& name) {
  ...
  for (auto const& p : effect.uniformBuffers) {
    if (p.second.bufferId)
      glBindBufferBase(GL_UNIFORM_BUFFER, p.second.bindingPoint, p.second.bufferId);
  }
...
void OpenGlRenderer::renderGlBuffer(GlRenderBuffer const& renderBuffer, Mat3F const& transformation) {
  for (auto const& vb : renderBuffer.vertexBuffers) {
    ...
    for (auto const& p : m_currentEffect->uniformBuffers) {
      if (p.second.bufferId)
        glBindBufferBase(GL_UNIFORM_BUFFER, p.second.bindingPoint, p.second.bufferId);
    }
...
#define APPLY_IF_VALID(Func, Arg1, ...) \
  if (Arg1 != -1)                       \
  Func(Arg1 __VA_OPT__(, ) __VA_ARGS__)
